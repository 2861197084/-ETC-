...
```
    ...
    实时流量统计              近期通行记录              历史通行记录
    当日热点卡口              待处理违规                历史违规归档
```

### 4.2 具体策略

| 数据类型 | 热数据（Redis） | 温数据（MySQL） | 冷数据（HBase） |
|----------|----------------|-----------------|-----------------|
| 通行记录 | 当天实时统计 | 最近7天明细 | 7天前历史数据 |
| 卡口流量 | 当前小时流量 | 当天统计 | 历史流量归档 |
| 违规记录 | - | 待处理记录 | 已处理归档 |
...
│                    Flink 定时任务                       │
├────────────────────────────────────────────────────────┤
│  每小时: Redis 实时统计 → MySQL 小时汇总表              │
│  每天凌晨: MySQL 7天前数据 → HBase 归档                 │
│  每月1号: HBase 超过1年数据 → HDFS 冷存储               │
└────────────────────────────────────────────────────────┘
```
...
│  │                        存储层 (Storage)                              │    │
│  │                                                                      │    │
│  │   MySQL 分片集群              HBase 集群               Redis 集群    │    │
│  │  ┌───────┬───────┐      ┌───────────────┐         ┌─────────────┐  │    │
│  │  │mysql-1│mysql-2│      │  hbase-master │         │redis-master │  │    │
│  │  │ ds_0  │ ds_1  │      ├───────┬───────┤         └──────┬──────┘  │    │
...
| **Node-5** | redis-cluster | Redis Master + Slave | 6379,6380 | 缓存 + 实时统计 |
...
public List<PassRecord> queryRecords(String plateNumber, Date startTime) {
    ...
    // 2. 判断是否温数据（7天内）
    if (isWithin7Days(startTime)) {
        int shard = hash(plateNumber) % 4;
        return mysqlShardService.query(shard, plateNumber, startTime);
    }
...
优化方式（快）：
1. 实时写入时，Flink 同步更新 Redis 计数器
2. 查询直接读 Redis: GET checkpoint:flow:CP001:20251218
→ O(1) 复杂度
```
...
Q: Redis 在架构中的作用？
A: 作为热数据缓存层，承接实时统计查询，降低数据库压力。
   实时流量统计、热点卡口排行等秒级响应都依赖 Redis。
...
echo "5. 启动 Redis 主从..."
docker-compose up -d redis-master redis-slave
sleep 5
...
1. **分片数量**：目前设计 4 分片，是否需要调整？
2. **HBase 预分区数**：16 个 Region 是否合适？
3. **冷数据阈值**：7 天是否合理，还是改成 3 天/14 天？
4. **是否需要实现所有组件**：考虑到时间，哪些可以简化或 Mock？
