...
```
HASH    stats:daily:{yyyyMMdd}             -> {totalCount, totalFee, totalDistance}
HASH    checkpoint:flow:{checkpointId}     -> {currentFlow, avgSpeed, updateTime}
ZSET    checkpoint:flow:history:{cpId}     -> score:timestamp, member:JSON{flow,speed}
```
