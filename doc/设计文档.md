...
### 2.4 统计缓存（Redis + Trino）

- **HBase 历史统计**：导入历史数据时计算每日/每卡口统计，缓存到 Redis（键：`etc:stats:hbase:daily:*`），TTL 24小时
- **MySQL 热数据统计**：优先使用 Redis 缓存；若缓存未命中，回退到 Trino 联邦查询（`jdbc:trino://trino:8080`）
- **统计 API**：`/api/stats/*` 自动选择数据源，前端车流量统计页面调用
- **时间格式兼容**：`HBaseTimeUtils` 支持多种时间格式（`yyyy/M/d H:mm:ss` 和 `yyyy-MM-dd HH:mm:ss`），解决数据格式不一致问题
